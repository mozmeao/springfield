...
class Command(BaseCommand):
    ...
    def _process_pages(self, dry_run, skip_strings, totals, page_ids_filter=None):
        ...
        for page_id, translation_key, locale_id in Page.objects.order_by("id").values_list("id", "translation_key", "locale_id"):
            groups[translation_key].append((page_id, locale_id))

        multi_locale_keys = [
            (translation_key, [pid for pid, _ in entries])
            for translation_key, entries in groups.items()
        ...
        for i, (translation_key, page_ids) in enumerate(multi_locale_keys, 1):
            # The page with the lowest pk (in this group of pages with the same
            # translation key) is considered the source page.
    ...
    def _process_snippets(self, dry_run, skip_strings, totals):
        for Model in TRANSLATABLE_SNIPPET_MODELS:
            groups = defaultdict(list)
            for inst_id, translation_key, locale_id in Model.objects.order_by("id").values_list("id", "translation_key", "locale_id"):
                groups[translation_key].append((inst_id, locale_id))

            multi_locale_keys = [
                (translation_key, [iid for iid, _ in entries])
                for translation_key, entries in groups.items()
                if len({locale_id for _, locale_id in entries}) > 1
            ]
