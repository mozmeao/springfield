...
def test_rule_on_non_canonical_page_is_rejected(wnp):
    """A rule attached to a nested variant is dead config and must not save.

    The guard itself is framework-side, in ``RoutingRule.clean()``, but it can only
    reject once a page type supplies ``is_routing_canonical`` — so the rejection is
    asserted here, against the first consumer that does.
    """
    grandchild = WhatsNewPage2026Factory(parent=wnp.variant, slug="145-b-nested", version="145", live=True)
    # A valid descendant target + match_all isolates the canonical-host error.
    rule = RoutingRule(page=wnp.variant, target=grandchild, match_all=True)
