...
class ImportRun(NamedTuple):
    """What one run of the command produced: what it said, and the CSVs it wrote."""
...
def run_import(tmp_path, *posts, **options):
    """Import `posts` (as built by post_xml), with both CSVs written inside tmp_path.

    Keyword options are passed to the command, so a test can add `dry_run=True` or `locale=...`.
    """
    ...
    call_command(
        ...
        str(write_xml(tmp_path, *posts)),
        url_map_out=str(url_map),
        warnings_out=str(warnings),
        stdout=stdout,
        stderr=stderr,
        **options,
    )
    return ImportRun(stdout.getvalue(), stderr.getvalue(), url_map, warnings)


def csv_rows(path):
    """The rows of a CSV the import wrote, as dicts. A run with nothing to report writes no file."""
    if not path.exists():
        return []
...
def make_command(dry_run=False):
    """A command wired up for calling one method directly, with its output captured."""
    command = Command()
...
def stderr_of(command):
    """What a directly-called command wrote to stderr."""
    return command.stderr._out.getvalue()
...
@pytest.fixture
def command():
    """A command wired up for calling one of its methods directly."""
    return make_command()
...
# -------------------------------------------------------------------------
# Telling alt text apart from a file name
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# HTML body -> specs: prose, headings and code
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# HTML body -> specs: videos and embeds
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# Specs -> StreamField blocks
# -------------------------------------------------------------------------


@responses.activate
def test_materialize_content_downloads_image_spec_into_media_block(command):
    mock_image_downloads()
    blocks = command.materialize_content([("text", "<p>Text</p>"), ("image", {"src": "https://example.com/a.png", "alt": "Alt"})], "A Test Post")
    assert [block[0] for block in blocks] == ["text", "media"]
    assert blocks[1][1][0][0] == "image"


@responses.activate
def test_materialize_content_image_with_caption_becomes_image_caption_block(command):
    mock_image_downloads()
    spec = ("image", {"src": "https://example.com/a.png", "alt": "Alt", "caption": "<p>A caption</p>"})
    blocks = command.materialize_content([spec], "A Test Post")
...
@responses.activate
def test_materialize_content_drops_image_whose_download_fails(command, no_retry_backoff):
    mock_failed_downloads()
    specs = [("text", "<p>Before</p>"), ("image", {"src": "https://example.com/a.png", "alt": ""}), ("text", "<p>After</p>")]
    blocks = command.materialize_content(specs, "A Test Post")
    assert [block[0] for block in blocks] == ["text", "text"]


@responses.activate
def test_materialize_content_drops_captioned_image_whose_download_fails(command, no_retry_backoff):
    mock_failed_downloads()
    spec = ("image", {"src": "https://example.com/a.png", "alt": "", "caption": "<p>A caption</p>"})
    assert command.materialize_content([spec], "A Test Post") == []


@responses.activate
def test_materialize_content_turns_inline_images_into_wagtail_embeds(command):
    mock_image_downloads()
...
@responses.activate
def test_materialize_content_inline_image_alt_falls_back_to_the_post_title(command):
    mock_image_downloads()
    blocks = command.materialize_content([("text", '<p><img src="https://example.com/a.png" alt=""/></p>')], "A Test Post")
    assert SpringfieldImage.objects.filter(title="A Test Post").exists()
    assert 'alt=""' in blocks[0][1]


@responses.activate
def test_inline_image_embed_drops_alt_that_names_a_file(command):
    mock_image_downloads()
...
@responses.activate
def test_content_image_description_comes_from_its_alt_attribute(command):
    mock_image_downloads()
    spec = ("image", {"src": "https://example.com/a.png", "alt": "The erase button in Firefox Focus", "caption": ""})
    command.materialize_content([spec], "A Test Post")
...
@responses.activate
def test_materialize_content_inline_image_keeps_its_link_wrapper(command):
    mock_image_downloads()
...
@responses.activate
def test_materialize_content_keeps_the_original_tag_when_an_inline_image_fails(command, no_retry_backoff):
    mock_failed_downloads()
...
@responses.activate
def test_materialize_content_text_without_images_is_untouched(command):
    blocks = command.materialize_content([("text", "<p>Just prose</p>")], "A Test Post")
    assert blocks == [("text", "<p>Just prose</p>")]
    assert len(responses.calls) == 0


@responses.activate
def test_materialize_content_video_spec_downloads_youtube_thumbnail_as_poster(command):
    mock_image_downloads()
    blocks = command.materialize_content([("video", {"url": "https://www.youtube.com/watch?v=abc123", "alt": "A video"})], "A Test Post")
...
@responses.activate
def test_materialize_content_gives_each_video_its_own_poster(command):
    """Two videos in one run must not share a thumbnail just because both URLs end in hqdefault.jpg."""
    mock_image_downloads()

    blocks = command.materialize_content(
        [
...
@responses.activate
def test_materialize_content_video_without_caption_falls_back_to_post_title_for_alt(command):
    mock_image_downloads()
    blocks = command.materialize_content([("video", {"url": "https://www.youtube.com/watch?v=abc123", "alt": ""})], "A Test Post")
...
@responses.activate
def test_materialize_content_passes_through_non_image_specs(command):
    blocks = command.materialize_content([("text", "<p>Hi</p>"), ("code", {"code": "print('hi')"})], "A Test Post")
...
def test_get_or_create_image_blank_url_returns_none(command):
    assert command.get_or_create_image("  ", "Some Title") is None


@responses.activate
def test_get_or_create_image_downloads_and_creates(command):
    mock_image_downloads()
    image = command.get_or_create_image("https://example.com/path/hero.jpg", "Hero")
    assert image.title == "Hero"
    assert "hero" in image.file.name


@responses.activate
def test_get_or_create_image_blank_title_falls_back_to_filename(command):
    mock_image_downloads()
    image = command.get_or_create_image("https://example.com/path/my-photo.jpg", "  ")
    assert image.title == "my-photo.jpg"


@responses.activate
def test_get_or_create_image_stores_a_youtube_poster_under_its_video_id(command):
    """Every YouTube thumbnail URL ends in the same 'hqdefault.jpg', so the id has to go in the name."""
    mock_image_downloads()

    image = command.get_or_create_image("https://img.youtube.com/vi/qxrVsN9kkog/hqdefault.jpg", "A video")

    assert "qxrVsN9kkog" in image.file.name


@responses.activate
def test_get_or_create_image_reuses_the_same_url_within_a_run_without_network_call(command):
    mock_image_downloads()

    first = command.get_or_create_image("https://example.com/whatever.png", "First Title")
    second = command.get_or_create_image("https://example.com/whatever.png", "A Different Title")
...
@responses.activate
def test_get_or_create_image_does_not_reuse_a_different_url_with_the_same_filename(command):
    """WordPress names files per upload month, so unrelated posts routinely share a basename."""
    mock_image_downloads()

    first = command.get_or_create_image("https://blog.mozilla.org/files/2024/11/image.png", "First")
    second = command.get_or_create_image("https://blog.mozilla.org/files/2025/10/image.png", "Second")
...
@responses.activate
def test_get_or_create_image_does_not_reuse_unrelated_image_with_same_title(command):
    mock_image_downloads()

    first = command.get_or_create_image("https://example.com/unrelated.png", "Screenshot")
    second = command.get_or_create_image("https://example.com/new-screenshot.png", "Screenshot")
...
@responses.activate
def test_get_or_create_image_retries_and_succeeds(command, no_retry_backoff):
    ...
    image = command.get_or_create_image(url, "Hero")
    assert image is not None
    assert len(responses.calls) == 2


@responses.activate
def test_get_or_create_image_gives_up_after_three_attempts(command, no_retry_backoff):
    mock_failed_downloads()
    image = command.get_or_create_image("https://example.com/hero.jpg", "Hero")
...
def test_get_or_create_image_unprocessable_file_warns_instead_of_raising(command, monkeypatch):
    ...
    assert command.get_or_create_image("https://example.com/huge.gif", "Huge") is None
    assert "could not process image https://example.com/huge.gif" in stderr_of(command)
    assert "cache resources exhausted" in stderr_of(command)


# -------------------------------------------------------------------------
# Topics, tags and authors
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# Running the command: validation and control flow
# -------------------------------------------------------------------------
...
def test_unknown_locale_raises_command_error(tmp_path, index_page):
    with pytest.raises(CommandError, match="No Locale found"):
        run_import(tmp_path, post_xml(), locale="xx-XX")
...
def test_missing_blog_index_page_raises_command_error(tmp_path):
    ...
    with pytest.raises(CommandError, match="No BlogIndexPage found"):
        run_import(tmp_path, post_xml(), locale="fr")


@responses.activate
def test_unsupported_post_type_is_skipped_and_does_not_abort_other_posts(tmp_path, index_page):
    mock_image_downloads()
    run = run_import(tmp_path, post_xml(id="1", slug="a-page", post_type="page"), post_xml(id="2", slug="a-test-post", post_type="post"))
...
def test_skip_already_imported_slug(tmp_path, index_page):
    mock_image_downloads()
    first = run_import(tmp_path, post_xml())
    assert BlogArticlePage.objects.count() == 1
    first.url_map.unlink()

    second = run_import(tmp_path, post_xml())

    assert BlogArticlePage.objects.count() == 1
    assert "skip (already imported): a-test-post" in second.stdout
    assert "Done. 0 imported, 1 skipped, 0 failed." in second.stdout
    # Nothing new to redirect, so the CSV is not (re)written.
    assert not second.url_map.exists()
...
def test_dry_run_reports_a_post_that_would_fail(tmp_path, index_page):
    ...
    run = run_import(tmp_path, post_xml(categories=""), dry_run=True)

    assert "has no Category" in run.stderr
    assert BlogArticlePage.objects.count() == 0
    assert "Done. 0 would be imported, 0 skipped, 1 failed." in run.stdout
...
def test_dry_run_writes_no_warnings_csv(tmp_path, index_page):
    ...
    run = run_import(tmp_path, post_xml(content=content), dry_run=True)

    # --dry-run writes nothing, but the warning still reaches the operator on stderr.
    assert not run.warnings.exists()
    assert "tiktok.com" in run.stderr


# -------------------------------------------------------------------------
# Running the command: the page it builds
# -------------------------------------------------------------------------
...
def test_successful_import_creates_page_with_expected_fields(tmp_path, index_page):
    mock_image_downloads()
    run = run_import(tmp_path, post_xml(tags="Privacy|Security"))
    ...
    with open(run.url_map, newline="") as fh:
...
def test_categories_topic_is_leaf_and_other_levels_become_tags(tmp_path, index_page):
    ...
    run_import(tmp_path, post_xml(categories="Firefox|Our Work>AI>AI Tech|Firefox>Firefox AI", tags="Security"))
...
def test_blank_categories_fails_the_post_instead_of_leaving_it_without_a_topic(tmp_path, index_page):
    ...
    run = run_import(tmp_path, post_xml(categories=""))

    assert not BlogArticlePage.objects.filter(slug="a-test-post").exists()
    assert "has no Category" in run.stderr
    assert "Done. 0 imported, 0 skipped, 1 failed." in run.stdout
...
def test_captioned_image_is_imported_as_an_image_caption_block(tmp_path, index_page):
    ...
    run_import(tmp_path, post_xml(content=content))
...
def test_youtube_iframe_is_imported_as_a_video_block(tmp_path, index_page):
    ...
    run = run_import(tmp_path, post_xml(content='<p>Watch:</p><iframe src="https://www.youtube.com/embed/abc123"></iframe>'))
...
def test_links_to_other_posts_in_the_import_point_at_their_new_pages(tmp_path, index_page):
    ...
    run_import(tmp_path, linking, target)
...
# -------------------------------------------------------------------------
# Running the command: the hero image
# -------------------------------------------------------------------------
...
def test_image_alt_text_comes_from_the_export_when_it_describes_the_image(tmp_path, index_page):
    ...
    run_import(tmp_path, post_xml(image_alt_text="A laptop showing the Firefox home page"))
...
def test_image_alt_text_is_left_blank_when_the_export_only_names_the_file(tmp_path, index_page):
    ...
    run_import(tmp_path, post_xml(image_alt_text="", image_title="SP_FX_Monitor_blogheader_01"))
...
def test_hero_image_caption_is_reported_because_the_page_has_nowhere_to_show_it(tmp_path, index_page):
    mock_image_downloads()
    run = run_import(tmp_path, post_xml(image_caption="Image source: imdb.com"))

    assert "Image source: imdb.com" in run.stderr
    assert "[caption]" in run.stderr


# -------------------------------------------------------------------------
# Running the command: what it reports
# -------------------------------------------------------------------------
...
def test_successful_import_writes_content_warnings_to_stderr(tmp_path, index_page):
    ...
    run = run_import(tmp_path, post_xml(content='<p>Watch:</p><iframe src="https://player.vimeo.com/video/123"></iframe>'))

    assert "https://player.vimeo.com/video/123" in run.stderr
...
def test_warnings_are_written_to_csv_against_the_post_they_came_from(tmp_path, index_page):
    ...
    run = run_import(tmp_path, post_xml(content=content))

    page = BlogArticlePage.objects.get(slug="a-test-post")
    rows = csv_rows(run.warnings)
    ...
    assert f"Wrote 3 warnings to {run.warnings}" in run.stdout
...
def test_failed_image_download_is_recorded_in_the_warnings_csv(tmp_path, index_page, no_retry_backoff):
    ...
    run = run_import(tmp_path, post_xml(content='<p>Text</p><img src="https://example.com/dead.png" alt="Alt">'))
...
def test_failed_post_records_its_warnings_and_the_failure(tmp_path, index_page):
    ...
    run = run_import(tmp_path, post_xml(content=content, categories=""))
...
def test_warnings_from_a_failed_post_do_not_leak_into_the_next_one(tmp_path, index_page):
    ...
    run = run_import(
        tmp_path, post_xml(id="1", slug="post-one", content=failing_content, categories=""), post_xml(id="2", slug="post-two", categories="Firefox")
    )
...
def test_no_warnings_csv_is_written_when_a_post_has_no_warnings(tmp_path, index_page):
    ...
    run = run_import(tmp_path, post_xml())
...
def test_url_map_generated_on_a_local_site_says_so(tmp_path, index_page):
    ...
    assert "localhost" in run.stdout
    assert "regenerate" in run.stdout.lower()
