...
def get_conditional_display_test_page() -> FreeFormPage2026:
    ...
    page.docs = (
        ...
        "<p>Use it sparingly: each conditional block adds page weight even when hidden, and large fallback gaps can leave the page "
        "feeling sparse on certain platforms. Always design a sensible &lsquo;show to all&rsquo; fallback so the page stays coherent "
        "if no condition matches.</p>"
    )
