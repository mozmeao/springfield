...
from springfield.firefox.referral.utils import CROCKFORD_ALPHABET, INVITE_CODE_LENGTH, REFERRAL_ID_LENGTH, is_canonical_crockford
...
def test_wrapper_round_trip_property_each_version(settings):
    ...
    for version in ("1", "2"):
        settings.REFERRAL_INVITE_CODE_ACTIVE_KEY_VERSION = version
        for _ in range(500):
            referral_id = "".join(rng.choice(CROCKFORD_ALPHABET) for _ in range(REFERRAL_ID_LENGTH))
            code = referral_id_to_invite_code(referral_id)
            assert code[0] == version
            assert invite_code_to_referral_id(code) == referral_id
...
def test_encode_is_deterministic():
    first = referral_id_to_invite_code("A7B9K2M4PXQRSTVW")
    second = referral_id_to_invite_code("A7B9K2M4PXQRSTVW")
    assert first == second


def test_decoded_id_is_canonical_crockford():
    code = referral_id_to_invite_code("A7B9K2M4PXQRSTVW")
    result = invite_code_to_referral_id(code)
    assert len(result) == REFERRAL_ID_LENGTH
...
def test_bitflip_ciphertext_char_decodes_to_a_different_id():
    ...
    assert len(result) == REFERRAL_ID_LENGTH and is_canonical_crockford(result)
