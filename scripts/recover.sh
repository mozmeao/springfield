...
#
# Database recovery for Springfield.
#
...
#                      --dump=gs://B/recovery/springfield-prod-202609041530.sql.gz \
...
# only exists in the production Springfield project, so a dev or stage recovery reaches
# across projects and needs bucket IAM rights there.
...
config() {
  ...
  INSTANCE="springfield-${TIER}-${ENV}-v1"
  DB_USER="springfield_rw_user"

  # US always; EU as well in stage and prod, on a separate cluster.
  REPLICAS=("${INSTANCE}-replica-0")
  [[ "$ENV" == dev ]] || REPLICAS+=("springfield-${TIER}-${ENV}-eu-replica-0")
}
...
FMT
  ...
  dump_uri="gs://${BUCKET}/recovery/springfield-${ENV}-${run_id}.sql.gz"
...
case "${1:-}" in
  ...
  -h | --help | "") sed -n '/^# Database recovery for Springfield\./,/^$/p' "$0" | sed 's/^#$//; s/^# \{0,1\}//' >&2 ;;
